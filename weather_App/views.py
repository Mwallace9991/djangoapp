...
def index(request):
    coordinates_url = "https://api.openweathermap.org/geo/1.0/direct?q={},{}&limit=1&appid={}"
    weather_url = "https://api.openweathermap.org/data/2.5/weather?lat={}&lon={}&appid={}&units=imperial"
...
def fetch_weather(city,state,coordinates_url,weather_url,forecast_url):
    ...
    for daily_data in forecast_response["list"][:5]:
        daily_forecasts.append(
            {
                "day": datetime.datetime.fromtimestamp(daily_data["dt"]).strftime("%A, %b %d, %I %p"),
